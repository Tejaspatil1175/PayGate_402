...
export default function VoiceAssistant() {
  ...
  return (
    <div className="min-h-[calc(100vh-5rem)] p-3 sm:p-6 max-w-5xl mx-auto w-full flex flex-col justify-between">
      <div className="flex-1 bg-transparent flex flex-col justify-between min-h-[520px]">
        <div ref={chatScrollRef} className="flex-1 overflow-y-auto p-2 sm:p-4 space-y-6 max-h-[620px]">
          {messages.map((msg) => {
              ...
              <div key={msg.id} className={`flex items-start gap-3.5 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
                ...
                <div className={`max-w-2xl space-y-1.5 ${isUser ? 'items-end' : 'items-start'}`}>
                  <div className={`rounded-3xl p-5 text-sm leading-relaxed transition shadow-2xs ${isUser ? 'bg-[#0F172A] text-white rounded-tr-sm' : msg.isError ? 'bg-rose-50/90 border border-rose-200 text-rose-900 rounded-tl-sm' : 'bg-[#F0F5FF] border border-[#DCE7FC] text-slate-800 rounded-tl-sm'}`}>
                    <p className="whitespace-pre-line text-slate-800 font-normal leading-relaxed">{msg.text}</p>
                    {msg.isWelcome && (
        ...
        <div className="pt-4 border-t border-slate-200/80 bg-white/50 backdrop-blur-md">
          {isRecording && (
            <div className="mb-3 flex items-center justify-between p-3 rounded-2xl bg-rose-50 border border-rose-200 text-rose-900 text-xs animate-in fade-in duration-150">
          ...
          <div className="flex items-center gap-3">
            <div className="relative group">
              ...
              <button type="button" onClick={isRecording ? stopRecording : startRecording} className={`relative p-3.5 rounded-2xl transition-all duration-300 shadow-md cursor-pointer flex items-center justify-center shrink-0 ${isRecording ? 'bg-rose-600 text-white animate-pulse' : 'bg-[#0B101B] text-indigo-400 hover:text-cyan-300'}`}>
                {isRecording ? <MicOff className="w-5 h-5" /> : <Mic className="w-5 h-5 text-[#818CF8]" />}
              </button>
            </div>
            <div className="flex-1 flex items-center gap-2 bg-white border border-slate-200/90 focus-within:border-indigo-400 focus-within:ring-2 focus-within:ring-indigo-100 rounded-2xl px-4 py-2.5 shadow-2xs transition">
              <input type="text" value={transcript} onChange={(e) => setTranscript(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSendText()} placeholder="Record or type your request (e.g., 'buy running shoes under 3000')..." className="flex-1 bg-transparent text-xs sm:text-sm text-slate-900 placeholder-slate-400 outline-none" />
              <div className="flex items-center gap-0.5 h-6 px-1 shrink-0">
                {audioLevel.map((height, idx) => (
                  <span key={idx} style={{ height: `${Math.max(4, height * 0.26)}px` }} className={`w-0.5 rounded-full transition-all duration-75 ${isRecording ? 'bg-rose-500 animate-pulse' : isSpeaking ? 'bg-indigo-500 animate-pulse' : idx % 3 === 0 ? 'bg-sky-400' : idx % 3 === 1 ? 'bg-indigo-500' : 'bg-purple-500'}`} />
                ))}
              </div>
            </div>
            <button type="button" onClick={() => handleSendText()} disabled={loading || !transcript.trim()} className="p-3 rounded-2xl bg-[#091322] hover:bg-[#0f1d33] text-sky-400 transition disabled:opacity-40 disabled:cursor-not-allowed shrink-0 shadow-xs cursor-pointer"><Send className="w-4 h-4" /></button>
            <button type="button" onClick={() => handleSendText()} disabled={loading || !transcript.trim()} className="px-4 py-2.5 rounded-2xl bg-[#596A82] hover:bg-[#475569] text-white font-bold text-xs transition disabled:opacity-40 disabled:cursor-not-allowed shrink-0 shadow-xs cursor-pointer hidden sm:block">Send</button>
          </div>
        </div>
      </div>
    </div>
